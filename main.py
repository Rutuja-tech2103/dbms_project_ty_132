...
def db_connect():
    ...
    host = "localhost"
    user = "userN"
    password = "RutujaBS#2103"  # Replace with your actual password
    database = "pharmacy_mang"
...
connection, cur = db_connect()
# Now you can use the 'conn' and 'cursor' objects to execute SQL queries and perform database operations.
# Create a connection to the MySQL database
conn, cursor = db_connect()
#conn = psycopg2.connect(dbname=DB_NAME, user="rutuja", password="r2103", host=DB_HOST)
#cur=conn.cursor()
...
def login():
    ...
    if uname == '' or pwd == '':
        ...
        try:
            DB_USER = uname
            DB_PASS = pwd
            # Use the database connection from db_connect function
            conn, cur = db_connect()
            if DB_USER[:3] == "med":
                employee()
            else:
...
def Loginform():
    ...
    login_screen.configure(bg="#FCF1C0")
    ...
    Label(login_screen,width="300", text="MedCare Login",font=("",15), bg="#B8DAD9",fg="white").pack()
    #Username Label
    Label(login_screen, text="Username * ",font=("",9), bg="#B8DAD9").place(x=20,y=60)
    #Username textbox
    Entry(login_screen, textvariable=username,width=25).place(x=105,y=62)
    #Password Label
    Label(login_screen, text="Password * ",font=("",9),bg="#B8DAD9").place(x=20,y=100)
    #Password textbox
    ...
    #Login button
    Button(login_screen, text="Login", width=13, height=1, bg="#83C760",font=("",10), command=login).place(x=105,y=155)
...
def admin():
    for i in login_screen.winfo_children():
       i.destroy()
    login_screen.geometry("900x500")
    frame0=Frame(login_screen,bg="#FFD3D6")
    frame0.pack()
...
def add_emp():
    ...
    l1 = Label(frame3, font=("", 10), text="EID")
    l1.grid(row=0, column=0, pady=10, padx=5)
    eid = StringVar(frame3)
    e1 = Entry(frame3, textvariable=eid)
    e1.grid(row=0, column=1, pady=10)
    
    l2 = Label(frame3, font=("", 10), text="Name")
    l2.grid(row=1, column=0, pady=10, padx=5)
    name = StringVar(frame3)
    e2 = Entry(frame3, textvariable=name)
    e2.grid(row=1, column=1, pady=10)
    
    l3 = Label(frame3, font=("", 10), text="Sex")
    l3.grid(row=2, column=0, pady=10, padx=5)
    sex = StringVar(frame3)
    OPTION_TUPLE = ("Male", "Female", "Other")
    o1 = OptionMenu(frame3, sex, *OPTION_TUPLE)
    o1.grid(row=2, column=1, pady=5)
    
    l4 = Label(frame3, font=("", 10), text="Contact_no")
    l4.grid(row=3, column=0, pady=10, padx=5)
    contact = StringVar(frame3)
    e4 = Entry(frame3, textvariable=contact)
    e4.grid(row=3, column=1, pady=10)
    
    l5 = Label(frame3, font=("", 10), text="Store_ID")
    l5.grid(row=4, column=0, pady=10, padx=5)
    store_id = StringVar(frame3)
    e5 = Entry(frame3, textvariable=store_id)
    e5.grid(row=4, column=1, pady=10)
    
    l6 = Label(frame3, font=("", 10), text="Salary")
    l6.grid(row=5, column=0, pady=10, padx=5)
    salary = StringVar(frame3)
    e6 = Entry(frame3, textvariable=salary)
    e6.grid(row=5, column=1, pady=10)
    
    l7 = Label(frame3, font=("", 10), text="Address")
    l7.grid(row=6, column=0, pady=10, padx=5)
    address = StringVar(frame3)
    e7 = Entry(frame3, textvariable=address, width=25)
    e7.grid(row=6, column=1, pady=10)
    
    b1 = Button(frame3, text="Submit", width=12, bg="orange", fg="white", command=lambda: submit3(eid.get(), name.get(), sex.get(), contact.get(), store_id.get(), salary.get(), address.get()))
...
def submit2(pid, name, quantity):
    try:
        ...
        prev_quantity = cur.fetchone()[0]
        
        if (int(prev_quantity) >= int(quantity)):
            amt = float(quantity) * unit_price
            cur.execute("insert into bill (pid, doc_id, drug_id, quantity, amt, store_id) values (%s, %s, %s, %s, %s, %s)",
                        (pid, doc_id, drug_id, quantity, amt, store_id))
            cur.execute("COMMIT")
            print("[+] Successfully inserted the bill")
        else:
            messagebox.showwarning("", "Not enough quantity in stock.")
    except Exception as e:
        messagebox.showwarning("", "Fields left blank or type error")
        cur.execute("END")
        add_bill()

def submit3(eid, name, sex, contact, store_id, salary, address):
    try:
        cur.execute("INSERT INTO employee (EID, empname, e_sex, e_contact, store_id, salary, e_address) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                    (eid, name, sex, contact, store_id, salary, address))
        connection.commit()
    except Exception as e:
        messagebox.showwarning("Error", "Fields left blank or type error: " + str(e))
        return
